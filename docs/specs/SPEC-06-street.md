...
## 목표
한 화면 전체에서 떨어지는 키오스크 25대를 파괴한다.

## 사용자 동작
화면 어디든 계속 떨어지는 키오스크를 25대 부수고, 낙하가 멈추면 남은 키오스크를 더 부수거나 오른쪽 공장 출구로 간다.

## 기술 설계
Street가 약 2초 주기 이벤트, 7번째 이벤트 묶음, 활성 12대 cap, stage별 카운트를 소유한다. 구름은 배경에 박지 않고 별도 hard-alpha 래스터 두 인스턴스로 이동·화면 밖 래핑한다.
...
## 테스트
- [x] 자동: 25대 설정·campaign E2E·적층 지지체 제거 회귀 테스트
- [x] 데스크톱: 낙하 상한·묶음·중단 smoke
- [ ] 모바일: 12대 상황 성능 실기기
- [x] 시각 검수: 고정 상점가 전 화면 x 낙하
- [x] 시각 검수: 배경 depth -10, 이동 구름 depth -9 및 화면 밖 래핑

## 증거
- 명령 및 결과: normal-balance E2E에서 Street → Factory 통과
- 스크린샷: street background 및 낙하 장면
- 성능 수치: interval 2000ms, cluster 3–5, cap 12, goal 25; stacked support removal drop 133.4px; grounded vx/vy 0; Android Chromium emulation 15-unit stress avg 61.5/p05 59.5 FPS
